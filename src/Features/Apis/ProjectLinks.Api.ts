...
export interface ProjectLink {
  ...
  label: string;
  url: string;
  type: 'github_fullstack' | 'live_demo' | 'documentation' | 'other';
  createdAt: string;
}
...
export const ProjectLinksApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getLinksByProjectId: builder.query<ProjectLink[], string>({
    ...
    syncProjectLinks: builder.mutation<ProjectLink[], { projectId: string; links: SyncLinkRequest[] }>({
    ...
    deleteProjectLink: builder.mutation<{ success: boolean }, string>({
      query: (linkId) => ({
