...
export const contentApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getContentByPage: builder.query({
      query: (pageName: string) => `content/page/${pageName}`,
      providesTags: (result) => 
        result 
          ? [...result.map(({ id }: any) => ({ type: 'Content' as const, id })), 'Content']
          : ['Content'],
    }),

    // Get a specific block (e.g., for a "Hero" specific editor)
    getContentByKey: builder.query({
      query: (key: string) => `content/key/${key}`,
...
export const {
  useGetContentByPageQuery,
  useGetContentByKeyQuery,
